    ...
    public function login()
    {
        if (isset($_POST['submit']))
        {
            ...
            if ($verify == 1) {
                ...
                $this->session->set_userdata('error', 'after your account request approved by admin you can login and communicate');
                redirect('auth/login');
            } else {
                /* Destory session if failed */
                // $this->session->sess_destroy();
                $this->session->set_userdata(['error' => 'Error!! Username and password are not valid!!']);
                redirect('auth/login');
            }
